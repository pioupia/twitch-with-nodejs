...
export class PanelComponent implements OnInit {
  ...
  private focus: any;
  private paramsToConfig: any;

  ngOnInit(): void {
    let active = true;
    let inListenTouche = false;
    let focus = !1;
    let paramsToConfig = !1;
    const isCookie = this.getCookie('data');
    const ecran: any = document.getElementById("ecran");
    const cam: any = document.getElementById("cam");

    let parameters: any = isCookie ?? {
      displayParams: 'e',
      retour: 'r',
      audioRetour: 'a',
      enDisStream: 's',
    ...
    for (const name in parameters.translator) {
      const theName: any = document.getElementById(name.replaceAll('App', ''));
      theName.innerHTML = `${theName.innerHTML}<br> Touche actuelle : ${parameters[parameters.translator[name]]}`
    }

    document.querySelectorAll('.configStream').forEach((name: any) => {
